...
func (b *BatchRequestHandler) ConstructRequests(batchRequest BatchRequestStructure, unsafe bool) ([]*http.Request, error) {
	...
	for i, requestDef := range batchRequest.Requests {
		...
		var absURL string
		if !unsafe {
			absUrlHeader := "http://localhost:" + strconv.Itoa(b.Conf.ListenPort)
...
func (b *BatchRequestHandler) HandleBatchRequest(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		b.doJSONWrite(w, 400, gw_api.APIError("Batch request malformed"))
		return
	}
	...
	if err != nil {
		b.doJSONWrite(w, 400, gw_api.APIError(fmt.Sprintf("Batch request creation failed , request structure malformed")))
		return
	}
...
func (b *BatchRequestHandler) doJSONWrite(w http.ResponseWriter, code int, obj interface{}) {
	...
	if code != 200 {
		job := b.Instrument.NewJob("SystemAPIError")
