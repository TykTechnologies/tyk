...
func (h *DefaultHealthChecker) Init(storeType storage.Handler) {
	...
	healthLog.Info("Initializing HealthChecker")
	h.storage = storeType
	h.storage.Connect()
}
...
func (h *DefaultHealthChecker) StoreCounterVal(counterType HealthPrefix, value string) {
	...
	healthLog.Debug("Adding Healthcheck to: ", searchStr)
	healthLog.Debug("Val is: ", value)
	...
	if value != "-1" {
		...
		healthLog.Debug("Set value to: ", value)
	}
...
func (h *DefaultHealthChecker) ApiHealthValues() (HealthCheckValues, error) {
	...
	for _, v := range vals {
		s := v.(string)
		healthLog.Debug("V is: ", s)
		splitValues := strings.Split(s, ".")
		if len(splitValues) > 1 {
			vInt, err := strconv.Atoi(splitValues[1])
			if err != nil {
				healthLog.Error("Couldn't convert tracked latency value to Int, vl is: ", err)
			} else {
				runningTotal += vInt
			}
		}
