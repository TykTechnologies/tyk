...
// configured scopeSource resolves to a global Scopes list, every
// request hitting the API must present a token whose merged scope
// set (across the configured ClaimNames) satisfies at least one of
// the configured OR-of-AND alternatives.
...
func (m *OAuth2Middleware) ProcessRequest(w http.ResponseWriter, r *http.Request, _ interface{}) (error, int) {
	...
	alternatives := globalScopeAlternatives(cfg.ScopeCheck)
	if len(alternatives) == 0 {
		// Global path has nothing to enforce.
		return nil, http.StatusOK
	}
