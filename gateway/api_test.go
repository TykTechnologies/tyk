...
// TestKeyHandler_RejectCustomKeyBelowMinTokenLength guards against the trap
// surfaced in TT-17065: at traffic time CheckSessionAndIdentityForValidKey
// rejects any inbound token where len(key) < MinTokenLength, but the POST
// create path historically accepted custom IDs under that bound and stored
// them, making the key unusable at auth time (403/AKI) with no create-path
// signal. This test locks the create path to return 400 for custom IDs
// strictly below MinTokenLength, while leaving autogenerated creates,
// keys exactly at MinTokenLength, and longer custom IDs untouched.
//
// Together with the operator fix in CheckSessionAndIdentityForValidKey
// (`<=` → `<`), this means the field name "min_token_length" now reads
// as it sounds: a key whose length equals min_token_length is accepted
// at both create and auth time.
func TestKeyHandler_RejectCustomKeyBelowMinTokenLength(t *testing.T) {
	ts := StartTest(func(globalConf *config.Config) {
		globalConf.MinTokenLength = 32
	})
	defer ts.Close()
	...
	cases := []struct {
		...
		{
			name:     "below_min_token_length_rejected",
			keyID:    "shortid12345678901234567890123", // 31 chars, strictly below 32
			wantCode: http.StatusBadRequest,
		},
		{
			name:     "equal_to_min_token_length_accepted",
			keyID:    "apigeecustomkey0123456789abcdefg", // 32 chars, == MinTokenLength
			wantCode: http.StatusOK,
		},
		{
			name:     "above_min_token_length_accepted",
			keyID:    "apigeecustomkey0123456789abcdefghh", // 34 chars, above MinTokenLength
			wantCode: http.StatusOK,
		},
	}
	...
	// Sanity: autogenerated creates (no custom ID) remain unaffected at
	// MinTokenLength=32 because generateToken produces an envelope above that.
	t.Run("autogenerated_key_unaffected", func(t *testing.T) {
		_, _ = ts.Run(t, test.TestCase{
			Method:    http.MethodPost,
			Path:      "/tyk/keys/create",
			Data:      body,
