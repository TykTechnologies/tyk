...
// prmBearerMethodsSupported is the fixed `bearer_methods_supported` value
// advertised in every PRM document Tyk serves from the new oauth2 block —
...
func (m *PRMMiddleware) ProcessRequest(w http.ResponseWriter, r *http.Request, _ interface{}) (error, int) {
	// New location wins: when oauth2.protectedResourceMetadata is
	// configured, it is the sole authority — the deprecated top-level
	// block is not consulted (the migration story handles coexistence
	// for APIs that only carry the old block).
...
// is the union of the operator-supplied list, the scheme's
// scopeCheck.Scopes baseline, and (by default) the auto-derived
// per-operation/per-primitive scope set — see OAS.OAuth2PRMScopesSupported.
// `bearer_methods_supported` is always ["header"].
