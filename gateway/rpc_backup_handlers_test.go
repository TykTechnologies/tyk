...
func TestDecompressPolicyBackup(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			input:    `[{"id":"test","name":"Test Policy","org_id":"test-org"}]`,
			wantErr:  false,
			expected: `[{"id":"test","name":"Test Policy","org_id":"test-org"}]`,
		},
		{
			name:     "Empty uncompressed data",
			input:    `[]`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result, err := ts.Gw.decompressPolicyBackup(tt.input)
	...
	t.Run("Compressed data round-trip", func(t *testing.T) {
		...
		original := `[{"id":"test","name":"Test Policy","org_id":"test-org","active":true}]`

		// Compress the data
		compressed := ts.Gw.compressPolicyBackup(original)
		...
		// Decompress it back
		decompressed, err := ts.Gw.decompressPolicyBackup(compressed)
		if err != nil {
			t.Fatalf("Failed to decompress: %v", err)
		}
