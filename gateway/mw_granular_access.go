...
func (m *GranularAccessMiddleware) ProcessRequest(w http.ResponseWriter, r *http.Request, _ interface{}) (error, int) {
	...
	if m.Spec.GraphQL.Enabled {
		if len(sessionVersionData.AllowedTypes) == 0 {
			return nil, http.StatusOK
		}

		gqlRequest := ctxGetGraphQLRequest(r)

		if !sessionVersionData.AllowedTypesEnabled {
			return nil, http.StatusOK
		}

		fieldRestrictionList := graphql.FieldRestrictionList{
			Kind:  graphql.AllowList,
			Types: sessionVersionData.AllowedTypes,
		}

		result, err := gqlRequest.ValidateFieldRestrictions(m.Spec.GraphQLExecutor.Schema, fieldRestrictionList, graphql.DefaultFieldsValidator{})
		if err != nil {
