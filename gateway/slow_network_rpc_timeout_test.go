...
func testReadinessDuringTimeouts(t *testing.T, ts *Test, _ bool, scenarioName string) {
...
func TestSlowNetworkRecovery(t *testing.T) {
	...
	fastDispatcher.AddFunc("Login", func(_, _ string) bool {
		return true // Fast login
	})
	fastDispatcher.AddFunc("GetApiDefinitions", func(_, _ *model.DefRequest) (string, error) {
		return "[]", nil // Fast API calls
	})
	fastDispatcher.AddFunc("GetPolicies", func(_, _ string) (string, error) {
		return "[]", nil // Fast policy calls
	})
