...
func TestCircuitBreaker5xxs(t *testing.T){
	...
	t.Run("Extended Paths", func(t *testing.T) {
		...
		ts.Run(t, []test.TestCase{
			{Path: "/error/500", Code: http.StatusInternalServerError},
			{Path: "/error/501", Code: http.StatusNotImplemented},
			{Path: "/error/502", Code: http.StatusBadGateway},
			{Path: "/error/500", Code: http.StatusServiceUnavailable},
			{Path: "/error/501", Code: http.StatusServiceUnavailable},
			{Path: "/error/502", Code: http.StatusServiceUnavailable},
		}...)
	})
}

func TestCircuitBreaker4xxs(t *testing.T){
	ts := StartTest()
	defer ts.Close()
	...
	t.Run("Extended Paths", func(t *testing.T) {
		...
		ts.Run(t, []test.TestCase{
			{Path: "/error/429", Code: http.StatusTooManyRequests},
			{Path: "/error/410", Code: http.StatusGone},
			{Path: "/error/409", Code: http.StatusConflict},
			{Path: "/error/429", Code: http.StatusServiceUnavailable},
			{Path: "/error/200", Code: http.StatusServiceUnavailable},
			{Path: "/error/409", Code: http.StatusServiceUnavailable},
		}...)
	})
}
