...
// dollarSecretToKVRef converts a matched $secret_* token's key into the
// whole-value kv:// reference the resolver understands, routing each legacy
// label to its registry store. The token regexes admit no '#', '{', '}' or ':'
// characters, so the key cannot alter the reference's structure.
