...
// TestDeepCopyBodySkip_MemoryBounded mirrors #8150's memory-proof technique:
// runtime.GC()+ReadMemStats deltas on a direct call, not OS RSS, not a full
// HTTP round trip. When the per-API passthrough flag is enabled, the reverse
// proxy's call-site guard must skip deepCopyBody entirely, keeping
// allocation bounded instead of multiplied.
...
// flag off, the reverse proxy must still deep-copy the body, byte-identical
// to master, even if the gateway-wide gate is on.
func TestDeepCopyBodyCallSite_PerAPIFlagOff_StillCopies(t *testing.T) {
	...
	assert.True(t, src.Body != trg.Body, "reverse proxy must still deep-copy when the per-API flag is off")
...
// TestHandleWrapperStreamRequestBody_MemoryBounded: with both the global
// gate and requestBodyPassthrough on, handleWrapper must skip the greedy
// nopCloseRequestBodyErr read for both Content-Length-framed and chunked
// bodies. Unlike copyRequest, nopCloseRequestBodyErr has no pre-existing
// chunked shortcut, so this is the branch that actually fixes the
// multiplier bug.
...
// TestPerAPIFlagOnly_MuxerStillBuffersReverseProxySkips covers the reverse
// combination from the tests above: with only the per-API flag on (global
// gate off), the muxer must still buffer the body -- it cannot see the
// per-API flag before routing -- while the reverse proxy skips its now-
// redundant copy without corrupting anything, since the body is already
// buffered.
func TestPerAPIFlagOnly_MuxerStillBuffersReverseProxySkips(t *testing.T) {
	t.Run("muxer still buffers when global gate is off", func(t *testing.T) {
	...
	t.Run("reverse proxy skips the now-redundant copy", func(t *testing.T) {
...
// and re-wraps the body itself before the reverse proxy is ever reached, so
// skipping deepCopyBody changes nothing observable for this request --
// streaming is inert here, not broken.
