...
func (h *HeaderInjector) HandleResponse(rw http.ResponseWriter, res *http.Response, req *http.Request, ses *user.SessionState) error {
	...
	if found {
		...
		for nKey, nVal := range hmeta.AddHeaders {
			setCustomHeader(res.Header, nKey, h.replaceTykVariables(req, nVal, false), ignoreCanonical)
		}
	}
	...
	for key, val := range vInfo.GlobalResponseHeaders {
		...
		setCustomHeader(res.Header, key, h.replaceTykVariables(req, val, false), ignoreCanonical)
	}
	...
	for header, v := range h.config.AddHeaders {
		setCustomHeader(res.Header, header, h.replaceTykVariables(req, v, false), ignoreCanonical)
	}
