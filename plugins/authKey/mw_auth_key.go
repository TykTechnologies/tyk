...
func initAuthKeyErrors() {
	gateway.TykErrors[ErrAuthAuthorizationFieldMissing] = config.TykError{
		Message: gateway.MsgAuthFieldMissing,
		Code:    http.StatusUnauthorized,
	}

	gateway.TykErrors[ErrAuthKeyNotFound] = config.TykError{
		Message: gateway.MsgApiAccessDisallowed,
		Code:    http.StatusForbidden,
	}

	gateway.TykErrors[ErrAuthCertNotFound] = config.TykError{
		Message: gateway.MsgApiAccessDisallowed,
		Code:    http.StatusForbidden,
	}

	gateway.TykErrors[ErrAuthKeyIsInvalid] = config.TykError{
		Message: gateway.MsgApiAccessDisallowed,
		Code:    http.StatusForbidden,
	}

	gateway.TykErrors[ErrAuthCertExpired] = config.TykError{
		Message: gateway.MsgCertificateExpired,
		Code:    http.StatusForbidden,
	}
}
...
func (k *AuthKey) setContextVars(r *http.Request, token string) {
	...
	if cnt := gateway.CtxGetData(r); cnt != nil {
		// Key data
		cnt["token"] = token
		gateway.CtxSetData(r, cnt)
	}
}
...
func (k *AuthKey) ProcessRequest(_ http.ResponseWriter, r *http.Request, _ interface{}) (error, int) {
	...
	if key != "" {
		...
		if time.Now().After(r.TLS.PeerCertificates[0].NotAfter) {
			return gateway.ErrorAndStatusCode(ErrAuthCertExpired)
		}

		key = k.Gw.GenerateToken(k.Spec.OrgID, certHash)
	} else {
		k.Logger().Info("Attempted access with malformed header, no auth header found.")
		return gateway.ErrorAndStatusCode(ErrAuthAuthorizationFieldMissing)
	}
	...
	case apidef.AuthToken, apidef.UnsetAuth:
		gateway.CtxSetSession(r, &session, updateSession, k.Gw.GetConfig().HashKeys)
		k.setContextVars(r, key)
		gateway.CtxSetSpanAttributes(r, k.Name(), []otel.SpanAttribute{
...
func (k *AuthKey) reportInvalidKey(key string, r *http.Request, msg string, errMsg string) (error, int) {
	...
	gateway.ReportHealthValue(k.Spec, gateway.KeyFailure, "1")

	return gateway.ErrorAndStatusCode(errMsg)
}
...
func AuthFailed(m gateway.TykMiddleware, r *http.Request, token string) {
	m.Base().FireEvent(gateway.EventAuthFailure, gateway.EventKeyFailureMeta{
		EventMetaDefault: gateway.EventMetaDefault{Message: "Auth Failure", OriginatingRequest: gateway.EncodeRequestToEvent(r)},
		Path:             r.URL.Path,
