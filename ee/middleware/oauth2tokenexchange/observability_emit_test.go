...
func TestProcessRequest_SuccessSpanMetricAudit(t *testing.T) {
	...
	idpSpan := findSpan(spans, "HTTP POST")
...
// TestProcessRequest_NoExchangeFiresEmitsNothing pins that a request for which
// no exchange target exists is a silent no-op: no span, no metric, no audit
// event — and the exchange is marked done so MCP fan-out sub-requests do not
// re-enter it.
func TestProcessRequest_NoExchangeFiresEmitsNothing(t *testing.T) {
	noTarget := provider("corpIdP", "http://idp.invalid", false)
	noTarget.DefaultTarget = nil
	...
	tests := []struct {
		...
		{"no providers", nil},
		{"providers but no resolvable target", []oas.OAuth2TokenExchangeProvider{noTarget}},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			st := exchangeState(noTarget)
			st.OASConfig.TokenExchange.Providers = tt.providers

			ctx, parent := otel.Tracer("test").Start(context.Background(), "request")
			r := httptest.NewRequest(http.MethodPost, "http://gw/api/tools", nil).WithContext(ctx)
			oauth2common.SetState(r, st)
			_, status := m.ProcessRequest(httptest.NewRecorder(), r, nil)
			parent.End()

			assert.Equal(t, http.StatusOK, status)
			assert.True(t, oauth2common.IsExchangeDone(r), "no-op must still mark the exchange done")
			assert.Nil(t, findSpan(sr.Ended(), exchangeSpanName), "no-op must not open an exchange span")
			assert.Empty(t, base.metrics)
			assert.Empty(t, base.events)
			assert.Empty(t, base.cacheHits)
		})
	}
}
...
func TestProcessRequest_CacheHitOmitsIdPSpan(t *testing.T) {
	...
	require.NotNil(t, findSpan(sr1.Ended(), "HTTP POST"), "first request is a miss")

	sr2 := installRecorder(t)
	processInTrace(t, m, exchangeState(p), sr2)

	// No IdP round-trip on the hit.
	assert.Nil(t, findSpan(sr2.Ended(), "HTTP POST"), "cache hit must not call the IdP")
