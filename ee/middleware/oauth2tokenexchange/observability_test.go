...
// TestEventPayload_CapsAzpFields pins that the authorized-party fields are
// length-capped on both the log line and the audit meta. SubjectAzp is a claim
// on the unverified inbound token and ExchangedAzp comes from the IdP response
// body, so neither value is trusted with unbounded size.
func TestEventPayload_CapsAzpFields(t *testing.T) {
	t.Parallel()

	p := okPayload()
	p.SubjectAzp = strings.Repeat("a", oauth2common.MaxIdPErrorBodyBytes+500)
	p.ExchangedAzp = strings.Repeat("b", oauth2common.MaxIdPErrorBodyBytes+500)

	maxLen := oauth2common.MaxIdPErrorBodyBytes + len("...(truncated)")
	for _, m := range []map[string]interface{}{p.LogFields(), p.AuditMeta()} {
		subject, _ := m["oauth2_subject_azp"].(string)
