...
func TestWrapDialer_DialsACachedIPv6Address(t *testing.T) {
	...
	for _, strategy := range []config.IPsHandleStrategy{config.PickFirstStrategy, config.RandomStrategy, config.NoCacheStrategy} {
		t.Run(string(strategy), func(t *testing.T) {
			manager := NewDnsCacheManager(strategy)
			manager.SetCacheStorage(&MockStorage{
				MockFetchItem: func(string) ([]string, error) { return []string{"::1", "::1"}, nil },
				MockGet:       func(string) (DnsCacheItem, bool) { return DnsCacheItem{}, false },
				MockSet:       func(string, []string) {},
				MockDelete:    func(string) {},
				MockClear:     func() {},
			})

			dial := manager.WrapDialer(&net.Dialer{Timeout: time.Second})
			conn, err := dial(context.Background(), "tcp", "svc6.example:"+port)
			if err != nil {
				t.Fatalf("dialling an AAAA-only host through the DNS cache failed: %v", err)
			}
			conn.Close()
		})
	}
