...
func TestMTLSConfig_IsExplicitlyConfigured(t *testing.T) {
	tests := []struct {
		name     string
		config   MTLSConfig
		expected bool
		...
		},
		{
			...
			expected: false, // enabled=true alone is not considered explicit configuration
		},
		{
			name: "cert file only",
			config: MTLSConfig{
		...
		{
			name: "CA cert IDs only",
			config: MTLSConfig{
				CACertIDs: []string{"ca-cert-id"},
			},
			expected: true,
		},
		{
		...
		{
			name: "insecure skip verify false (default)",
			config: MTLSConfig{
				InsecureSkipVerify: false,
			},
			expected: false, // false is the default, so not considered explicit
		},
		{
			name: "full file config",
			config: MTLSConfig{
				Enabled:  true,
				CertFile: "/path/to/cert.crt",
				KeyFile:  "/path/to/key.key",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := tt.config.IsExplicitlyConfigured()
			assert.Equal(t, tt.expected, result, "IsExplicitlyConfigured() = %v, want %v", result, tt.expected)
		})
	}
}
