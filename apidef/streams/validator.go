...
func loadSchemas() error {
	...
	loadBento := func() error {
		...
		bentoValidators[bento.DisabledValidator] = bento.NewDisabledConfigValidator()
		return nil
	}
...
func validateBentoConfiguration(document []byte, bentoValidatorKind bento.ValidatorKind) error {
	...
	// If using disabled validator, skip validation
	if bentoValidatorKind == bento.DisabledValidator {
		return nil
	}
...
func ValidateOASObjectWithConfig(documentBody []byte, oasVersion string, disableValidator bool) error {
	...
	if disableValidator {
		validatorKind = bento.DisabledValidator
	}
