...
func TestValidateTykStreams_BentoConfigValidation_Additional_Properties(t *testing.T) {
	...
	t.Run("with disabled validator", func(t *testing.T) {
		err := ValidateOASObjectWithBentoConfigValidator(document, "3.0.3", bento.DisabledValidator)
		require.NoError(t, err)
		...
		err = ValidateOASObjectWithBentoConfigValidator(amqpDocument, "3.0.3", bento.DisabledValidator)
		require.NoError(t, err)
	})
}
