...
func TestFixtures(t *testing.T) {
	...
	// Read the embedded fixtures.yml file.
	data, err := fixtures.ReadFile("fixtures.yml")
	assert.NoError(t, err, "failed to read fixtures.yml from embed FS")

	// Unmarshal the YAML content into a FixtureDocument.
	err = yaml.Unmarshal(data, &doc)
	assert.NoError(t, err, "failed to unmarshal fixtures YAML")
			...
			for idx, tc := range fixture.Tests {
				t.Run(fmt.Sprint(idx), func(t *testing.T) {
					...
					default:
						...
						outputFlatMap, err := maps.Flatten(tc.Output)
						assert.NoError(t, err)
