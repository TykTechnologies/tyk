...
type DerivedTool struct {
	...
	// The map is adapter metadata and is not serialized.
...
type DerivedPrimitive struct {
	// Type is the MCP primitive type represented by this catalogue entry.
	Type string `json:"type"`
	// Tool is populated when Type is MCPPrimitiveTypeTool.
...
type DeriveWarning struct {
	...
	Operation string
	// Method is the source HTTP method for the warning.
	Method string
	// Path is the source OpenAPI path for the warning.
	Path string
	// Reason explains why the operation was not exposed.
	Reason string
}
...
type MCPToolView struct {
	// Tools is the caller-facing tool catalogue for one proxy.
