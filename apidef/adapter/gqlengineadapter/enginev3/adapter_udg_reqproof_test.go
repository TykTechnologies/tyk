...
// Verifies: SYS-REQ-104, SW-REQ-079
// SW-REQ-079:nominal:nominal
// SW-REQ-079:boundary:nominal
// SW-REQ-079:error_handling:nominal
// SW-REQ-079:error_handling:negative
// SW-REQ-079:determinism:nominal
func TestEngineV3UniversalDataGraphEngineConfigPreservesLocalDatasourceConfiguration(t *testing.T) {
	apiDef := reqproofEngineV3UDGAPIDefinition(t)
	...
	require.Equal(t, plan.TypeFields{{TypeName: "Query", FieldNames: []string{"restSearch"}}}, dataSources[0].RootNodes)
	...
	require.Equal(t, plan.TypeFields{{TypeName: "Query", FieldNames: []string{"graphItem"}}}, dataSources[1].RootNodes)
	...
	require.Equal(t, plan.TypeFields{{TypeName: "Query", FieldNames: []string{"graphAsRest"}}}, dataSources[2].RootNodes)
	...
	require.Equal(t, plan.TypeFields{{TypeName: "Subscription", FieldNames: []string{"topicEvents"}}}, dataSources[3].RootNodes)
	...
	repeatedAdapter := &UniversalDataGraph{
		ApiDefinition:             reqproofEngineV3UDGAPIDefinition(t),
		HttpClient:                httpClient,
		StreamingClient:           streamingClient,
		subscriptionClientFactory: reqproofEngineV3SubscriptionClientFactory{},
	}
...
func reqproofEngineV3UDGAPIDefinition(t *testing.T) *apidef.APIDefinition {
	t.Helper()

	return &apidef.APIDefinition{
		GraphQL: apidef.GraphQLConfig{
			Schema: `type Query {
  restSearch(term: String!, limit: Int): [String]
				...
				DataSources: []apidef.GraphQLEngineDataSource{
					{
						Kind:       apidef.GraphQLEngineDataSourceKindREST,
						RootFields: []apidef.GraphQLTypeFields{{Type: "Query", Fields: []string{"restSearch"}}},
						Config: reqproofEngineV3RawJSON(t, apidef.GraphQLEngineDataSourceConfigREST{
					...
					},
					{
						...
						Config: reqproofEngineV3RawJSON(t, apidef.GraphQLEngineDataSourceConfigGraphQL{
					...
					},
					{
						...
						Config: reqproofEngineV3RawJSON(t, apidef.GraphQLEngineDataSourceConfigKafka{
							BrokerAddresses:      []string{"localhost:9092"},
							Topics:               []string{"events.{{.arguments.topic}}"},
							GroupID:              "reqproof.group",
							ClientID:             "reqproof.client",
...
func reqproofEngineV3RawJSON(t *testing.T, value interface{}) json.RawMessage {
	t.Helper()
