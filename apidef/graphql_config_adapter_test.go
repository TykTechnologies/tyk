...
func TestGraphQLConfigAdapter_EngineConfigV2(t *testing.T) {
	run := func(t *testing.T, inputJSON string) (*graphql.EngineV2Configuration, error) {
	...
	runWithError := func(inputJSON string, expectedErr error) func(t *testing.T) {
		return func(t *testing.T) {
			_, err := run(t, inputJSON)
			assert.Error(t, err)
			assert.Equal(t, expectedErr, err)
		}
	}

	runWithoutError := func(inputJSON string, expectedEngineV2ConfigBuilder func(t *testing.T) *graphql.EngineV2Configuration) func(t *testing.T) {
		return func(t *testing.T) {
			engineV2Conf, err := run(t, inputJSON)
	...
	t.Run("should convert graphql v2 config to engine config v2",
		runWithoutError(graphqlEngineV2ConfigJson,
