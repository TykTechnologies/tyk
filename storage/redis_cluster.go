...
func (r *RedisCluster) Connect(_ context.Context) bool {
	return true
}
...
func (r *RedisCluster) SetExp(ctx context.Context, keyName string, timeout int64) error {
	if err := r.up(); err != nil {
		return err
	}
...
func (r *RedisCluster) SetKey(ctx context.Context, keyName, session string, timeout int64) error {
...
func (r *RedisCluster) SetRawKey(ctx context.Context, keyName, session string, timeout int64) error {
	if err := r.up(); err != nil {
		return err
	}
...
func (r *RedisCluster) GetKeys(ctx context.Context, filter string) []string {
	...
	fnFetchKeys := func(nctx context.Context, client *redis.Client) ([]string, error) {
		values := make([]string, 0)

		iter := client.Scan(nctx, 0, searchStr, 0).Iterator()
		for iter.Next(ctx) {
	...
	case *redis.ClusterClient:
		...
		go func() {
			err = v.ForEachMaster(ctx, func(nctx context.Context, client *redis.Client) error {
				values, err := fnFetchKeys(nctx, client)
				if err != nil {
					return err
				}
...
func (r *RedisCluster) DeleteScanMatch(ctx context.Context, pattern string) bool {
	...
	fnScan := func(nctx context.Context, client *redis.Client) ([]string, error) {
		values := make([]string, 0)

		iter := client.Scan(nctx, 0, pattern, 0).Iterator()
		for iter.Next(ctx) {
			values = append(values, iter.Val())
		}
...
func (r *RedisCluster) AppendToSetPipelined(ctx context.Context, key string, values [][]byte) {
	...
	if storageExpTime := int64(config.Global().AnalyticsConfig.StorageExpirationTime); storageExpTime != int64(-1) {
		...
		if exp == -1 {
			r.SetExp(ctx, key, storageExpTime)
		}
	}
}
...
func (r *RedisCluster) SetRollingWindow(ctx context.Context, keyName string, per int64, value_override string, pipeline bool) (int, []interface{}) {
	log.Debug("Incrementing raw key: ", keyName)
	if err := r.up(); err != nil {
		log.Debug(err)
		return 0, nil
...
func (r *RedisCluster) GetKeyPrefix(ctx context.Context) string {
	return r.KeyPrefix
}
