...
func Test_Dummy_SetKeyEx(t *testing.T) {
	for _, tc := range []struct {
		name string
		fn   func(*DummyStorage, string, string, int64) error
	}{
		{"SetKeyEx", (*DummyStorage).SetKeyEx},
		{"SetRawKeyEx", (*DummyStorage).SetRawKeyEx},
	} {
		storage := NewDummyStorage()

		t.Run(tc.name, func(t *testing.T) {
			t.Run("does not set if key already exist", func(t *testing.T) {
				const key = "key"

				err := tc.fn(storage, key, "value", 0)
			...
			t.Run("modifies key if one already exist", func(t *testing.T) {
				const key = "key"

				err := storage.SetKey(key, "value0", 0)
				assert.NoError(t, err)

				err = tc.fn(storage, "key", "value1", 0)
				assert.NoError(t, err)
