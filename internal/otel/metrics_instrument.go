...
// exchangeDurationBuckets defines histogram bucket boundaries (in seconds)
// sized for the IdP round-trip on a token exchange.
...
// Token-exchange instrument names. Operators wire dashboards and alerts to
// these, and the metrics tests assert on them, so they are named once here.
const (
	exchangeMetricRequests = "tyk.oauth2.exchange.requests"
	exchangeMetricDuration = "tyk.oauth2.exchange.duration"
	exchangeMetricCacheHit = "tyk.oauth2.exchange.cache_hit"
	actorMetricRequests    = "tyk.oauth2.actor.requests"
	actorMetricDuration    = "tyk.oauth2.actor.duration"
)
...
func NewMetricInstruments(provider tykmetric.Provider, logger *logrus.Logger) *MetricInstruments {
	...
	exchangeDuration, err := provider.NewHistogram(
		...
		"Time to obtain the exchanged token, by outcome and provider",
		"s",
