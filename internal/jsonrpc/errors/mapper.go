...
func SelectJSONRPCCode(httpStatus int, classification *tykerrors.ErrorClassification, protocolContext *mcp.ProtocolContext) int {
	if !protocolContext.IsModern() {
		return MapHTTPStatusToJSONRPCCode(httpStatus)
	}
...
func codeForKind(kind errorKind, modern bool) int {
	...
	if modern {
		return map[errorKind]int{
			errorKindServer: CodeModernServerError, errorKindAuth: CodeModernAuthRequired,
			errorKindAccess: CodeModernAccessDenied, errorKindQuota: CodeModernQuotaExceeded,
			errorKindRateLimit: CodeModernRateLimitExceeded, errorKindIPBlocked: CodeModernIPBlocked,
			errorKindUpstream: CodeModernUpstreamError,
		}[kind]
	}
	return legacy
}
