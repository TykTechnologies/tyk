...
func TestSelectJSONRPCCode_ProtocolNamespaceAndClassification(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "authentication", status: http.StatusForbidden, class: tykerrors.NewErrorClassification(tykerrors.AKI, "invalid_key"), legacyCode: CodeAccessDenied, modernCode: CodeModernAuthRequired},
		...
		{name: "upstream", status: http.StatusInternalServerError, class: tykerrors.NewErrorClassification(tykerrors.UCF, "connection_failed"), legacyCode: mcp.JSONRPCInternalError, modernCode: CodeModernUpstreamError},
		{name: "quota", status: http.StatusForbidden, class: tykerrors.NewErrorClassification(tykerrors.QEX, "quota_exceeded"), legacyCode: CodeAccessDenied, modernCode: CodeModernQuotaExceeded},
		{name: "IP blocked", status: http.StatusForbidden, class: tykerrors.NewErrorClassification(tykerrors.IPB, "ip_blocked"), legacyCode: CodeAccessDenied, modernCode: CodeModernIPBlocked},
	}
	...
	t.Run("standard codes do not move namespaces", func(t *testing.T) {
		...
		assert.Equal(t, CodeModernServerError, SelectJSONRPCCode(http.StatusInternalServerError, nil, modern))
	})
