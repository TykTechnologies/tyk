...
func NewStorage(cfg *config.StorageOptionsConf, externalServicesConfig *config.ExternalServiceConfig) redis.UniversalClient {
	log.Debugf("[ExternalServices] Creating Redis client for rate limiter")
	...
	if cfg.UseSSL {
		log.Debug("[ExternalServices] Configuring TLS for Redis connection")
...
func createTLSConfig(cfg *config.StorageOptionsConf, externalServicesConfig *config.ExternalServiceConfig) *tls.Config {
	...
	if externalServicesConfig != nil && externalServicesConfig.Storage.MTLS.Enabled {
		log.Debug("[ExternalServices] Using external services storage configuration for Redis TLS")
		...
		if storageConfig.CertFile != "" && storageConfig.KeyFile != "" {
			...
			if err != nil {
				log.WithError(err).Error("Failed to load external services storage client certificate for Redis")
			} else {
				tlsConfig.Certificates = []tls.Certificate{cert}
				log.Debug("Loaded Redis client certificate from external services storage configuration")
			}
		}
		...
		if storageConfig.CAFile != "" {
			...
			if err != nil {
				log.WithError(err).Error("Failed to load external services storage CA certificate for Redis")
			} else {
				caCertPool := x509.NewCertPool()
				if !caCertPool.AppendCertsFromPEM(caCert) {
					log.Error("Failed to parse external services storage CA certificate for Redis")
				} else {
					tlsConfig.RootCAs = caCertPool
					log.Debug("Loaded Redis CA certificate from external services storage configuration")
				}
			}
		}
		...
		if storageConfig.TLSMinVersion != "" {
			if version, ok := getTLSVersion(storageConfig.TLSMinVersion); ok {
				...
				log.Debugf("[ExternalServices] Redis TLS MinVersion set to: %s", storageConfig.TLSMinVersion)
			}
		}
		if storageConfig.TLSMaxVersion != "" {
			if version, ok := getTLSVersion(storageConfig.TLSMaxVersion); ok {
				tlsConfig.MaxVersion = version
				log.Debugf("[ExternalServices] Redis TLS MaxVersion set to: %s", storageConfig.TLSMaxVersion)
			}
		}

		// External services config takes priority, skip legacy configuration
		log.Debug("[ExternalServices] Redis TLS configuration completed using external services config")
		return tlsConfig
	}

	// Legacy certificate loading (only if external services config not used)
	log.Debug("[ExternalServices] Using legacy storage configuration for Redis TLS")

	// Load CA certificate if provided
	if cfg.CAFile != "" {
		caCert, err := os.ReadFile(cfg.CAFile)
		if err != nil {
			log.WithError(err).Error("Failed to load legacy CA certificate for rate limiter Redis")
		} else {
			caCertPool := x509.NewCertPool()
			if !caCertPool.AppendCertsFromPEM(caCert) {
				log.Error("Failed to parse legacy CA certificate for rate limiter Redis")
			} else {
				tlsConfig.RootCAs = caCertPool
				log.Debug("Loaded Redis CA certificate from legacy storage configuration")
			}
		}
	}
	...
	if cfg.CertFile != "" && cfg.KeyFile != "" {
		...
		if err != nil {
			log.WithError(err).Error("Failed to load legacy client certificate and key for rate limiter Redis")
		} else {
			tlsConfig.Certificates = []tls.Certificate{cert}
			log.Debug("Loaded Redis client certificate from legacy storage configuration")
		}
	}
	...
	if cfg.TLSMinVersion != "" {
		if version, ok := getTLSVersion(cfg.TLSMinVersion); ok {
			tlsConfig.MinVersion = version
			log.Debugf("[ExternalServices] Redis legacy TLS MinVersion set to: %s", cfg.TLSMinVersion)
		}
	}
	if cfg.TLSMaxVersion != "" {
		if version, ok := getTLSVersion(cfg.TLSMaxVersion); ok {
			tlsConfig.MaxVersion = version
			log.Debugf("[ExternalServices] Redis legacy TLS MaxVersion set to: %s", cfg.TLSMaxVersion)
		}
	}

	log.Debug("[ExternalServices] Redis TLS configuration completed using legacy config")
	return tlsConfig
}
...
func getTLSVersion(version string) (uint16, bool) {
	...
	default:
		log.Warnf("Unknown TLS version: %s", version)
		return 0, false
	}
}
