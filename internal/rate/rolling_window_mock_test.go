...
func TestRollingWindow_MockCount(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	defer cancel()

	now, per, key := time.Now(), int64(5), "test-key"

	t.Run("no-error", func(t *testing.T) {
		conn, mock := redismock.NewClientMock()
		rl := rate.NewRollingWindow(conn)

		mock.ExpectZCount(key, rl.StartPeriod(now, per), "+inf").SetVal(3)

		val, err := rl.Count(ctx, key, now, per)
