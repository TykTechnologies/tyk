...
handler behavior in `gateway/handler_error.go`.
...
and the `TestOverrideErrors` coverage in `gateway/gateway_test.go`. It covers
Tyk error map default and configured override lookup, template response selection
for JSON/XML/SOAP requests, JSON-RPC error response selection and helper
delegation when MCP JSON-RPC routing state exists, request-side error override
response writing and fallback, direct override body writing, analytics status
selection, access-log status selection, latency invariants, and local
metrics/health recording for exercised paths.
