...
The executable evidence is `storage/dummy_test.go`. It covers dummy storage
construction, multi-key and raw-key reads, key set/get/delete behavior,
wildcard key deletion and listing, list append/range/remove behavior, existence
checks across data and index maps, connect success, and explicit panic behavior
for methods that are currently unsupported by DummyStorage.
