...
The executable evidence is `internal/httpclient/*_test.go`. It covers the
decomposed local outputs for service configuration detection and merging,
service-specific client constructors, service timeouts and transport settings,
proxy selection and bypass behavior, mTLS file and certificate-store error
handling, mTLS error classification, certificate-store loading, and JWK fetching
through a supplied HTTP client and parser.
