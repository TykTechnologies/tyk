...
2. Scope onboarding path:
   ...
   - Recent increments include `pkg/validator` custom-policy-ID validation,
     `dnscache` local DNS cache storage, and `internal/service/core`
     upstream-auth request-context helpers. They reduce the visible scope gap
     but do not change the remaining warning disposition.
