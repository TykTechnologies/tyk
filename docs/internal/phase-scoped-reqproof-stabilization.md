...
2. Scope onboarding path:
   ...
   - Recent increments include `pkg/validator` custom-policy-ID validation,
     ...
     `internal/errors` diagnostic classification, and `internal/oasutil` OAS
     path/server helpers. They reduce the visible scope gap but do not change
     the remaining warning disposition.
