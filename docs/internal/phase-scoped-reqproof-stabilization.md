...
2. Scope onboarding path:
   ...
   - Recent increments include `pkg/validator` custom-policy-ID validation,
     ...
     `internal/service/gojsonschema` and `internal/errors` facades. They reduce
     the visible scope gap but do not change the remaining warning disposition.
