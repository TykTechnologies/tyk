...
2. Scope onboarding path:
   ...
   - Recent increments include `pkg/validator` custom-policy-ID validation,
     ...
     `pkg/schema` OAS visitor/unicode-escape helpers, and the
     `internal/middleware` custom middleware enablement helper. They reduce the
