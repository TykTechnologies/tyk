...
2. Scope onboarding path:
   ...
   - Recent increments include `pkg/validator` custom-policy-ID validation,
     ...
     `apidef/oas` error-override shape helpers, and the `apidef/oas` schema
     example extraction helper. They reduce the visible scope gap but do not
     change the remaining warning disposition.
