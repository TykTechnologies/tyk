...
2. Scope onboarding path:
   ...
   - Recent increments include `pkg/validator` custom-policy-ID validation and
     `dnscache` local DNS cache storage. They reduce the visible scope gap but
     do not change the remaining warning disposition.
