...
2. Scope onboarding path:
   ...
   - Recent increments include `pkg/validator` custom-policy-ID validation,
     ...
     helper shape, the `apidef/oas` readable-duration alias, and the
     `apidef/oas` error-override shape helpers. They reduce the visible scope
