...
2. Scope onboarding path:
   ...
   - Recent increments include `pkg/validator` custom-policy-ID validation,
     ...
     `pkg/errpack` typed diagnostic error helpers, and the
     `internal/service/newrelic` observability adapter helpers. They reduce the
