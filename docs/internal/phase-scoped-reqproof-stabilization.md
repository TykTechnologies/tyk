...
2. Scope onboarding path:
   ...
   - Recent increments include `pkg/validator` custom-policy-ID validation,
     ...
     `apidef/oas` endpoint tracking shape helper, and the `apidef/oas` utility
     helper shapes. They reduce the visible scope gap but do not change the
